...
PERSISTED_ENVIRONMENT_KEYS = ('VIREA_HOME', 'UV_CACHE_DIR', 'HF_HOME')
GENERATED_ENVIRONMENT_HEADER = (
    '# Generated by VIREA; rerun configure-virea.sh to change the data root.'
)

def persisted_virea_environment():
    config_root = Path(
        os.environ.get('XDG_CONFIG_HOME') or Path.home() / '.config'
    ).expanduser().resolve(strict=False)
    environment_file = config_root / 'virea' / 'environment.sh'
    if not environment_file.is_file():
        return {}, str(environment_file), (
            'WSL persistent data-root configuration was not found; run '
            'scripts/configure-virea.sh inside this distribution'
        )
    try:
        ...
        return {}, str(environment_file), (
            'WSL persistent data-root configuration could not be read: '
            + type(exc).__name__
        )
    if len(content) > 65536:
        return {}, str(environment_file), (
            'WSL persistent data-root configuration exceeds 64 KiB'
        )
    lines = content.splitlines()
    if not lines or lines[0] != GENERATED_ENVIRONMENT_HEADER:
        return {}, str(environment_file), (
            'WSL persistent data-root configuration has an invalid generator header'
        )
    values = {}
    for raw_line in lines[1:]:
        ...
        if not line.startswith('export '):
            return {}, str(environment_file), (
                'WSL persistent data-root configuration contains an unsupported statement'
            )
        assignment = line[len('export '):]
        name, separator, raw_value = assignment.partition('=')
        if name not in PERSISTED_ENVIRONMENT_KEYS:
            continue
        if not separator or name in values:
            return {}, str(environment_file), (
                'WSL persistent data-root configuration contains a duplicate or invalid '
                + name
            )
        if (
            len(raw_value) < 2
            ...
            return {}, str(environment_file), (
                'WSL persistent data-root configuration contains a non-literal ' + name
            )
        try:
            tokens = shlex.split(line, comments=False, posix=True)
        except ValueError:
            return {}, str(environment_file), (
                'WSL persistent data-root configuration contains invalid shell quoting'
            )
        if len(tokens) != 2 or tokens[0] != 'export':
            return {}, str(environment_file), (
                'WSL persistent data-root configuration contains a compound statement'
            )
        parsed_name, parsed_separator, value = tokens[1].partition('=')
        if parsed_name != name or not parsed_separator:
            return {}, str(environment_file), (
                'WSL persistent data-root configuration contains an invalid assignment'
            )
        candidate = Path(value)
        if not candidate.is_absolute() or '..' in candidate.parts:
            return {}, str(environment_file), (
                'WSL persistent data-root configuration contains a relative or traversing '
                + name
            )
        try:
            ...
            return {}, str(environment_file), (
                'WSL persistent data-root configuration contains an invalid ' + name
            )
    if set(values) != set(PERSISTED_ENVIRONMENT_KEYS):
        return {}, str(environment_file), (
            'WSL persistent data-root configuration is incomplete'
        )
    home = Path(values['VIREA_HOME'])
    data_root = home.parent
    expected = {
        'VIREA_HOME': data_root / 'home',
        'UV_CACHE_DIR': data_root / 'uv-cache',
        'HF_HOME': data_root / 'home' / 'cache' / 'huggingface',
    }
    if any(Path(values[name]) != path for name, path in expected.items()):
        return {}, str(environment_file), (
            'WSL persistent data-root configuration does not match the generated layout'
        )
    return values, str(environment_file), None
...
persisted_environment, environment_file, environment_warning = (
    persisted_virea_environment()
)
environment_is_persisted = bool(persisted_environment)
home = Path(persisted_environment.get('VIREA_HOME') or (
    Path(os.environ['XDG_DATA_HOME']) / 'virea'
    if os.environ.get('XDG_DATA_HOME')
    else Path.home() / '.local' / 'share' / 'virea'
)).expanduser().resolve(strict=False)
if not persisted_environment:
    cache_root = Path(
...
print(json.dumps({
    ...
    'environment_is_persisted': environment_is_persisted,
...
def _probe_wsl_execution_domain(
    *,
    ...
    tools["managed_data_root_status"] = (
        "configured"
        if payload.get("environment_is_persisted")
    ...
    if payload.get("environment_is_persisted"):
        ...
        tools.update(
            {
                "managed_uv_cache_dir": str(environment["UV_CACHE_DIR"]),
                "managed_hf_home": str(environment["HF_HOME"]),
                "managed_environment_file": str(payload["environment_file"]),
            }
        )
    return ExecutionDomainReport(
        ...
        tools=tools,
        warnings=(
            (str(payload["environment_warning"]),)
            if payload.get("environment_warning")
            else ()
        ),
    )
