...
def _format_quality_table(quality: dict, sample_id: str = "") -> str:
    ...
    if bbox_min and bbox_max:
        lines.append(
            f"  BBox: [{', '.join(f'{v:.3f}' for v in bbox_min)}]"
    ...
    if gc and gc.get("total_frames"):
        ...
        lines.append(
            f"    Floating:    {gc['floating_frames']:>5}/{gc['total_frames']} "
            f"({gc['floating_ratio'] * 100:5.1f}%)"
        )
        lines.append(
            f"    Penetrating: {gc['penetrating_frames']:>5}/{gc['total_frames']} "
            f"({gc['penetrating_ratio'] * 100:5.1f}%)"
        )
        lines.append(
            f"    Foot height: [{gc.get('min_foot_height_m', 0):.4f}m, "
    ...
    if rde:
        ...
        if rde.get("status") == "incompatible_shapes":
            lines.append(
                f"  [Retarget] incompatible shapes "
            ...
            lines.append(
                f"    Mean: {rde.get('overall_mean_rad', 0):.6f} rad "
                f"({rde.get('overall_mean_deg', 0):.4f} deg, {mean_pct:.4f}%)"
            )
            lines.append(
                f"    Max:  {rde.get('overall_max_rad', 0):.6f} rad "
    ...
    if per_bone:
        ...
        lines.append(
            f"  {'Bone':<22} {'Mean(rad)':>11} {'Max(rad)':>11} {'Std(rad)':>11} {'MaxDeg':>8}"
        )
        lines.append(f"  {'-' * 68}")
...
def _aggregate_dataset_quality(items: list[dict[str, Any]]) -> str:
    ...
    passed_items = [
        it for it in items if it.get("quality") and it["quality"].get("status")
    ]
    if not passed_items:
    ...
    if direction_summaries:
        ...
        avg_mean_rad = sum(d["overall_mean_rad"] for d in direction_summaries) / len(
            direction_summaries
        )
        ...
        lines.append(
            f"    Avg mean: {avg_mean_rad:.6f} rad ({math.degrees(avg_mean_rad):.4f} deg, {avg_mean_pct:.4f}%)"
        )
        lines.append(
            f"    Global max: {worst_max_rad:.6f} rad ({math.degrees(worst_max_rad):.4f} deg, {worst_max_pct:.4f}%)"
        )
        lines.append("")
        lines.append(
            "  === SOURCE MOTION CHARACTERISTICS (inherent data properties) ==="
        )

    if ground_contacts:
        avg_float = sum(gc["floating_ratio"] for gc in ground_contacts) / len(
            ground_contacts
        )
        avg_pen = sum(gc["penetrating_ratio"] for gc in ground_contacts) / len(
            ground_contacts
        )
        lines.append("")
        lines.append("  [Ground Contact]")
    ...
    if velocities:
        ...
        lines.append("  [Velocity]")
        lines.append(
            f"    Avg mean speed: {avg_speed:.3f} m/s | Global max: {max_speed:.3f} m/s"
        )
    ...
    if symmetries:
        ...
        lines.append("")
        lines.append("  [Symmetry (source motion L/R asymmetry)]")
        lines.append(
            f"    Avg max asymmetry: {avg_sym * 100:.2f}% | Worst: {max_sym * 100:.2f}%"
        )
    ...
    if all_per_bone:
        ...
        lines.append(
            f"  {'Bone':<36} {'AvgMean(rad)':>13} {'AvgMax(rad)':>13} {'AvgMaxDeg':>10}"
        )
        lines.append(f"  {'-' * 76}")
        aggregated = []
        ...
        for bone, avg_mean, avg_max, avg_max_deg in aggregated[:22]:
            lines.append(
                f"  {bone:<36} {avg_mean:>13.6f} {avg_max:>13.6f} {avg_max_deg:>10.4f}"
            )
    ...
    if sample_max_dir_errors:
        ...
        lines.append(
            f"  {'#':<4} {'MaxErr(rad)':>12} {'MaxErr(deg)':>12}  {'Sample ID'}"
        )
        lines.append(f"  {'-' * 72}")
...
def _cmd_process(args: argparse.Namespace) -> None:
    ...
    if not paths.raw_root.exists():
        custom = input(
            f"  raw_root not found: {paths.raw_root}\n  Enter path (or empty to abort): "
        ).strip()
        if not custom:
    ...
    limit = (
        ...
        if args.limit_per_dataset and args.limit_per_dataset > 0
        else 1_000_000_000
    )
    tasks = batch.collect_tasks(
        datasets=datasets_filter, query=args.query, limit_per_dataset=limit
    )
    workers = args.workers if args.workers > 0 else default_worker_count()

    legacy_pickle_datasets = {task.dataset for task in tasks} & {
        "grab",
        "susuinteracts",
    }
    if (
        legacy_pickle_datasets
        and os.getenv("VIREA_ALLOW_TRUSTED_RAW_PICKLE", "").strip() != "1"
    ):
    ...
    def on_progress(result) -> None:
        ...
        short_id = (
            result.task.sample_id[-40:]
            if len(result.task.sample_id) > 40
            else result.task.sample_id
        )
        print(
    ...
    print(
        f"  Processed: {report_dict['processed']} | "
    ...
    for ds_name, ds_items in sorted(by_dataset.items()):
        ...
        print(
            f"  Total: {len(ds_items)} | OK: {n_ok} | Skipped: {n_skip} | Failed: {n_fail}"
        )
        print(f"{'-' * 70}")
...
def _cmd_serve(args: argparse.Namespace) -> None:
    ...
    print(f"\n  Data source: {source}")
    print(
        f"  Raw root:    {paths.raw_root} {'(OK)' if paths.raw_root.exists() else '(NOT FOUND)'}"
    )
    print(f"  Processed:   {paths.processed_root}")
    if host and port:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    proc.add_argument(
        "--datasets", nargs="*", default=[], help="Datasets to process (default: all)."
    )
    ...
    proc.add_argument(
        ...
        type=int,
        default=None,
        help="Frame cap per clip (omit for full).",
    )
    proc.add_argument(
    ...
    proc.add_argument(
        "--force", action="store_true", help="Reprocess even if artifacts exist."
    )
    ...
    srv.add_argument(
        "--host", default="", help="Bind host. Can also be set with VIREA_SERVE_HOST."
    )
    srv.add_argument(
        "--port",
        type=int,
        default=None,
        help="Bind port. Can also be set with VIREA_SERVE_PORT.",
    )
