...
class HumanML3DAdapter(BaseDatasetAdapter):
    ...
    def _caption_annotations(
        self, sample_id: str, caption: str, fps: float, duration_sec: float
    ) -> list[dict]:
    ...
    def discover(self, limit: int = 50, query: str = "") -> list[SampleRef]:
        ...
        for path in self._parquet_files():
            ...
            for row_idx, row in table.iterrows():
                ...
                if not (
                    self._matches(sample_id, query)
                    or self._matches(caption, query)
                    or self._matches(name, query)
                ):
                    continue
                frame_count = (
                    int(meta.get("num_frames", 0)) if isinstance(meta, dict) else None
                )
                duration = (
                    float(meta.get("duration", 0.0)) if isinstance(meta, dict) else None
                )
                samples.append(
                    self._sample(
                        sample_id,
                        path,
                        "humanml3d_263d_parquet",
                        "humanml3d_263d",
                        fps=20.0,
                        ...
                        metadata={
                            ...
                            "dataset_profile": "humanml3d_263d",
                        },
                    )
                )
    ...
    def load(self, sample_id: str, max_frames: int | None = None) -> RawClip:
        ...
        if len(parts) != 3:
            raise ValueError(
                f"HumanML3D sample_id must be split/shard/row, got {sample_id}"
            )
        ...
        caption = str(row["caption"])
        frame_count = (
            int(meta.get("num_frames", motion.shape[0]))
            if isinstance(meta, dict)
            else motion.shape[0]
        )
        duration = (
            float(meta.get("duration", frame_count / 20.0))
            if isinstance(meta, dict)
            else frame_count / 20.0
        )
        sample = self._sample(
            sample_id,
            path,
        ...
        annotations = self._caption_annotations(
            sample_id, caption, fps=20.0, duration_sec=duration
        )
        return RawClip(
            sample=sample,
            motion={"motion": motion, "fps": 20.0},
