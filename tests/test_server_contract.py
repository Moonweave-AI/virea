...
def test_server_is_local_first_and_write_api_is_fail_closed(monkeypatch) -> None:
    ...
    assert all(
        "raw_root" not in item for item in payload["available_data_sources"].values()
    )
...
def test_preview_default_preserves_full_clip_and_absolute_path_redaction(
...
def test_project_preview_limit_supports_full_or_explicit_configuration(
    tmp_path,
) -> None:
    base = {
        "paths": {},
    ...
    assert (
        ProjectPaths(
            config={**base, "runtime": {"preview_max_frames": None}}
        ).preview_max_frames
        is None
    )
    assert (
        ProjectPaths(
            config={**base, "runtime": {"preview_max_frames": 1800}}
        ).preview_max_frames
        == 1800
    )


def test_project_data_defaults_stay_under_virea_home(tmp_path, monkeypatch) -> None:
    home = tmp_path / "managed-home"
    monkeypatch.setenv("VIREA_HOME", str(home))
    monkeypatch.delenv("VIREA_RAW_ROOT", raising=False)
    monkeypatch.delenv("VIREA_PROCESSED_ROOT", raising=False)

    demo = ProjectPaths(data_source="demo")
    full = ProjectPaths(data_source="full")

    assert demo.raw_root == home / "data-sources" / "demo" / "raw"
    assert demo.processed_root == home / "data-sources" / "demo" / "processed"
    assert full.raw_root == home / "data-sources" / "full" / "raw"
    assert full.processed_root == home / "data-sources" / "full" / "processed"


def test_sample_catalog_exposes_profile_fps_without_claiming_native_fps(
    tmp_path,
) -> None:
    raw_root = tmp_path / "raw"
    ...
    assert (
        payload["preview_fps_provenance"] == "dataset_profile:beat_bvh_full75_runtime"
    )
    assert payload["source_path"] == "raw/beat/pose/1/clip.bvh"


def test_sidecar_read_is_content_addressed_contained_and_nosniff(
    tmp_path, monkeypatch
) -> None:
    raw_root = tmp_path / "raw"
    processed_root = tmp_path / "processed"
    ...
    wrong = "0" * 64
    assert (
        client.get(f"/api/artifacts/sidecars/{wrong}?data_source=full").status_code
        == 404
    )
    assert (
        client.get("/api/artifacts/sidecars/not-a-digest?data_source=full").status_code
        == 404
    )


def test_sidecar_read_falls_back_to_verified_on_demand_cache(
    tmp_path, monkeypatch
) -> None:
    fake_registry = SimpleNamespace(
        paths=SimpleNamespace(
