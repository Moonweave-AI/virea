...
1. 有 positions：按profile unit、root origin和world basis映射63-joint positions，再做whole-hand position fitting。
2. 无 positions：用官方columns/local 6D解码local matrices；复现`process_batch_data`的local quaternion swizzle与pelvis correction，并使用官方`template_susu_retarget_63nodes.bvh`的完整body/hand source topology与rest offsets做MTA63 FK，再把同帧positions送入同一whole-hand fitter。

两条路径都不能把 local rotation 再当 world rotation转换一次。Position fitting 恢复 swing direction，不唯一恢复 twist。

两条路径的body、wrist与可观测finger swing都统一从positions fitting得到，不能把source direct fingers挂到另一个fitted wrist frame。Rotation-only 6D只用于固定source template上的MTA63 FK，不作为canonical direct rotation prior。单子骨axial twist与无fingertip的distal leaf继续标为unobservable；未校准本地变体继续fail-closed。
