...
| 执行环境 | 分别选择系统执行域、精确 Runtime 和资源 profile；历史选择只是默认值，不会静默强制。 |
| 部署 | 匹配的 READY 快照默认复用且不重新下载；不同执行环境会建立独立部署并保留旧快照。 |
| 生成 | 依次显示提交、模型推理和结果制品收集；成功时只显示 job/result ID 摘要。 |
| 浏览器 | 可选启动同步显示源骨架与最终 VRM 的本地工作台。 |

进度条只在真实完成一个阶段边界时前进。下载、Runtime 构建和模型推理无法预知精确字节/迭代进度时，
会显示活动指示器与已用时间，不伪造百分比。交互模式不会打印完整 RAW JSON；失败只显示错误码、主要原因、
下一步和证据目录，完整事务仍保存在 `VIREA_HOME/state` 与 `VIREA_HOME/logs`。下面的显式子命令继续保持
机器可读 JSON，供自动化和高级诊断使用。
