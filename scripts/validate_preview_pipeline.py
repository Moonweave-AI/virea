...
def validate_source(data_source: str, max_frames: int, persist: bool) -> dict[str, Any]:
    ...
    for dataset in registry.keys():
        ...
        processed = processed_pipeline.preview(
            dataset,
            sample_id,
    ...
    return sanitize_report_paths(
        {
            ...
            "raw_root_status": (
                "configured" if registry.paths.raw_root.exists() else "missing"
            ),
            "processed_root": f"data-source/{data_source}/processed",
            "processed_root_status": (
                "configured" if registry.paths.processed_root.exists() else "missing"
            ),
            "max_frames": max_frames,
            "persist": persist,
            "reports": reports,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Validate the raw-to-processed preview pipeline."
    )
    parser.add_argument(
        "--data-source",
        choices=[*AVAILABLE_DATA_SOURCES, "all"],
        default="demo",
    )
    ...
    report = {
        "schema_version": "virea.preview_pipeline_validation.v1.0.0",
        "sources": [
            validate_source(source, max_frames=args.max_frames, persist=args.persist)
            for source in sources
        ],
    }
    report["passed"] = all(
        source_report["passed"] for source_report in report["sources"]
    )
