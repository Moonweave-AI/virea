...
from .backend import CmdmBackend

MODEL_ID = "cmdm-humanml3d"
PLUGIN_VERSION = "0.1.0"
RUNTIME_ID = "cmdm-humanml3d-cu128"
...
def _resolve_motion_length_parameters(
    ...
    if seconds_value is not None:
        ...
        if rounded_frames % 4:
            raise WorkerFailure(
                "INVALID_REQUEST",
                "seconds must resolve to a multiple of 4 CMDM frames (0.2 seconds)",
            )
        motion_length_frames = rounded_frames
    return motion_length_frames, seconds


class CmdmHumanML3DPlugin:
    """Managed real-checkpoint Worker for CMDM HumanML3D."""
    ...
    def __init__(
        ...
        *,
        ...
        self._backend = CmdmBackend(roots)
    ...
    def metadata(self) -> WorkerMetadata:
        return WorkerMetadata(
            ...
            resources={
                "accelerator": "nvidia_or_cpu",
                "min_vram_gib": 6.0,
                "min_ram_gib": 8.0,
                "memory_strategies": ["cuda_full", "cpu"],
                "active_memory_strategy": os.getenv(
                    "VIREA_MEMORY_STRATEGY", "cuda_full"
                ),
                "resource_profile": os.getenv("VIREA_RESOURCE_PROFILE", "cuda-full"),
                "model_revision": CMDM_MODEL_REVISION,
                "source_revision": CMDM_SOURCE_REVISION,
                "distilbert_revision": DISTILBERT_REVISION,
                "humanml3d_revision": HUMANML3D_REVISION,
                "native_feature_dim": 263,
                "native_fps": FPS,
                "causal_vae_temporal_stride": 4,
                "cancel_semantics": (
    ...
    def cancel(self, job_id: str) -> None:
        # The released whole-sequence sampler has no safe mid-kernel hook.
        # VIREA's process supervisor may terminate this isolated Worker.
        return None
    ...
    def infer(self, request: WorkerInferRequest, context: WorkerContext) -> ModelResult:
        ...
        if request.request.task != "text_to_motion":
            raise WorkerFailure(
                "INVALID_TASK", "CMDM Worker supports only text_to_motion"
            )
        ...
        try:
            ...
            raise WorkerFailure(
                ...
                f"CMDM inference failed: {type(exc).__name__}: {exc}",
            ) from exc
        ...
        native_path = staging / "source_cmdm_vector263_denormalized.npy"
        mean_path = staging / "cmdm_humanml3d_mean.npy"
        std_path = staging / "cmdm_humanml3d_std.npy"
        np.save(native_path, generated.denormalized_vector263, allow_pickle=False)
        ...
        run_metadata = {
            "schema_version": "virea.cmdm_generation_metadata.v1.0.0",
            "job_id": request.job_id,
            "model": {
                "id": self.model_id,
                "repository": "https://github.com/lycorp-jp/CMDM",
                "source_revision": CMDM_SOURCE_REVISION,
                "checkpoint_revision": CMDM_MODEL_REVISION,
                "text_encoder_revision": DISTILBERT_REVISION,
                "humanml3d_revision": HUMANML3D_REVISION,
                "variant": "CMDM-Causal-DiT-MAC-VAE",
            },
            "runtime": {
            ...
            "sampling": {
                "prediction": "causal_flow_velocity",
                "schedule": "released_pyramid",
                "sampling_timesteps": 50,
                "causal_vae_stride": 4,
                "text_encoder": "distilbert-base-uncased",
                "inverse_normalization": "official_HumanML3D_Mean_Std",
            },
            "output": {
                "frame_count": generated.generated_frames,
                "fps": FPS,
                "dtype": str(generated.denormalized_vector263.dtype),
                "shape": list(generated.denormalized_vector263.shape),
        ...
        return ModelResult(
            ...
            model=ModelIdentity(
                id=self.model_id,
                plugin_version=PLUGIN_VERSION,
                upstream_repository="https://github.com/lycorp-jp/CMDM",
                upstream_revision=CMDM_SOURCE_REVISION,
                runtime_id=os.getenv("VIREA_RUNTIME_ID", RUNTIME_ID),
                artifact_manifest_id="cmdm-humanml3d-pinned-bundle",
            ),
            ...
            native=NativeMotionDescriptor(
                ...
                artifacts=(
                    ArtifactRef(
                        name="source_cmdm_vector263_denormalized",
            ...
            provenance=GenerationProvenance(
                ...
                generation_parameters={
                    ...
                    "motion_length_frames": motion_length_frames,
                    "seconds": seconds,
                    # The production result validator treats ``frame_count`` as
                    # the cross-model provenance contract.  Keep the
                    # CMDM-specific name as an additional upstream diagnostic,
                    # but always attest the canonical key as well.
                    "frame_count": frame_count,
                    "generated_frames": frame_count,
                    "cfg": cfg,
                    "fps": FPS,
                    "sampling_schedule": "pyramid",
                    "sampling_timesteps": 50,
                    "causal_vae_stride": 4,
                    **device_facts,
                },
                sources=(
                    SourceRevision(
                        repository="https://github.com/lycorp-jp/CMDM",
                        revision=CMDM_SOURCE_REVISION,
                        release="CVPR 2026",
                    ),
                    SourceRevision(
                        repository="ly-corporation/CMDM",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="virea-cmdm-worker")
...
def main() -> None:
    ...
    serve_plugin(
        CmdmHumanML3DPlugin(roots, model_id=args.model_id),
        host=args.host,
        port=args.port,
