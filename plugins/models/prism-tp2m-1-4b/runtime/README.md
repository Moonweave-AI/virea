...
Runtime 0.1.6 recognizes both the official `model.safetensors` component name and the standard Diffusers `diffusion_pytorch_model.safetensors` name. It creates a meta-device model skeleton, verifies every state key and tensor shape, then stages and converts one tensor at a time through CPU before direct final-device installation. It never uses Accelerate checkpoint dispatch, never loads the complete archive directly to CUDA, never renames or copies the checkpoint, never uses pickle, and never applies a whole-model dtype cast. Worker bytecode writes are disabled so importing the pinned source cannot mutate its integrity tree.
