...
from .backend import ReMoMaskBackend

MODEL_ID = "remomask-humanml3d"
PLUGIN_VERSION = "0.2.0"
RUNTIME_ID = "remomask-humanml3d-cu128"
...
def resolve_motion_length(
    ...
    if frames is not None and frames % 4:
        raise WorkerFailure(
            "INVALID_REQUEST", "motion length must be a multiple of 4 ReMoMask frames"
        )
    return frames, seconds


class ReMoMaskHumanML3DPlugin:
    def __init__(
        ...
        self._backend = ReMoMaskBackend(roots, Path(cache_root))
    ...
    def metadata(self) -> WorkerMetadata:
        ...
        return WorkerMetadata(
            ...
            tasks=("text_to_motion", "retrieval_augmented_text_to_motion"),
            ...
            resources={
                ...
                "min_vram_gib": None if cpu else 12.0,
                "min_ram_gib": 16.0 if cpu else 12.0,
                ...
                "source_revision": REMOMASK_SOURCE_REVISION,
                "model_revision": REMOMASK_MODEL_REVISION,
                "clip_revision": OPENAI_CLIP_REVISION,
                "native_feature_dim": 263,
                "native_fps": FPS,
                "retrieval_database": "pinned_upstream_32_row_part_motion_database",
    ...
    def infer(self, request: WorkerInferRequest, context: WorkerContext) -> ModelResult:
        ...
        if request.request.task not in {
            "text_to_motion",
            "retrieval_augmented_text_to_motion",
        }:
            raise WorkerFailure(
                "INVALID_TASK", "ReMoMask Worker supports text-to-motion tasks only"
            )
        ...
        if "retrieval_database" in request.request.input:
            raise WorkerFailure(
                "INVALID_REQUEST",
                "custom retrieval databases are not accepted; this release uses the pinned upstream database",
            )
        ...
        retrieval_top_k = _integer(
            parameters.get("retrieval_top_k", 1),
        ...
        try:
            ...
            raise WorkerFailure(
                ...
                f"ReMoMask inference failed: {type(exc).__name__}: {exc}",
            ) from exc
        ...
        generation_parameters = {
            ...
            "time_steps": time_steps,
            "retrieval_top_k": retrieval_top_k,
            "retrieved_motion_ids": list(generated.retrieved_motion_ids),
            "cond_scale": cond_scale,
            "temperature": temperature,
            "topkr": topkr,
            "residual_cond_scale": residual_cond_scale,
        ...
        metadata_path.write_text(
            json.dumps(
                {
                    "schema_version": "virea.remomask_generation_metadata.v1.0.0",
                    "job_id": request.job_id,
                    "model": {
                        "id": self.model_id,
                        "repository": "https://github.com/AIGeeksGroup/ReMoMask",
                        "source_revision": REMOMASK_SOURCE_REVISION,
                        "model_revision": REMOMASK_MODEL_REVISION,
                        "clip_revision": OPENAI_CLIP_REVISION,
                    },
                    ...
                    "output": {
                        ...
                        "normalization": "inverse_official_remomask_humanml3d_mean_std",
                    },
                },
                ensure_ascii=False,
                indent=2,
        ...
        return ModelResult(
            ...
            model=ModelIdentity(
                id=self.model_id,
                plugin_version=PLUGIN_VERSION,
                upstream_repository="https://github.com/AIGeeksGroup/ReMoMask",
                upstream_revision=REMOMASK_SOURCE_REVISION,
                runtime_id=os.getenv("VIREA_RUNTIME_ID", RUNTIME_ID),
                artifact_manifest_id="remomask-humanml3d-pinned-hf-bundle",
            ),
            ...
            provenance=GenerationProvenance(
                ...
                sources=(
                    SourceRevision(
                        repository="https://github.com/AIGeeksGroup/ReMoMask",
                        revision=REMOMASK_SOURCE_REVISION,
                        release="arXiv 2025",
                    ),
                    SourceRevision(
                        repository="AIGeeksGroup/ReMoMask",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="virea-remomask-worker")
